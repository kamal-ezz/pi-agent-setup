...
- Main-agent Auto mode by default: routine actions run automatically while destructive, security-critical, or `AGENTS.md`-violating actions require approval with a persistent in-harness warning and desktop notification; Shift+Tab cycles blue Auto, yellow read-only Plan, and red bypass-all modes
- A mode-colored arrow prefixes the input bar; the terminal tab shows an animated spinner during active turns. While a turn is running, a one-line editor hint makes delivery explicit: Enter steers the next turn, Alt+Enter queues a follow-up until the current run finishes, and Alt+Up restores queued messages for editing. Conversation history uses a stable in-app viewport: mouse wheel and Page Up/Down scroll without streaming output snapping the view, while the editor and footer stay pinned. Hold Shift while dragging for native terminal text selection. Agent responses support natural, sparing Unicode emoji. `/clear` starts a fresh empty session, while Ctrl+C or Cmd+C restores an interrupted prompt without a notification or abort popup, and a second press exits Pi
