 ...
 * - interrupt clears the queue and aborts; close() emits the child
 *   session_shutdown hook and disposes the session.
 */
...
import { createEventChannel, type SubagentBackend, type SubagentSession } from "../backend.ts";
import type {
  SpawnTask,
  SubagentMeta,
  TranscriptPart,
...
async function makePiSession(task: SpawnTask): Promise<SubagentSession> {
  ...
  try {
    ...
    // ModelRegistry is the sync facade extensions receive; createAgentSession
    // wants the underlying ModelRuntime, which the facade holds privately.
    // Fall back to the SDK's default runtime if that internal ever moves.
    const modelRuntime = (registry as unknown as { runtime?: ModelRuntime })
      .runtime;
  ...
  return {
    ...
    send: (text) => {
      ...
      if (session.isStreaming) {
        ...
        return session.steer(text).then(
          () => undefined,
          (error) => {
    ...
    interrupt: async () => {
      ...
      // manager bounds this call at 5s and force-disposes on timeout.
